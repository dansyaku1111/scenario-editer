...
### ノードラベル名機能
全てのノードタイプに `name` フィールドが追加されました。このラベル名はノードの種類名の上部に表示され、シナリオの視認性を大幅に向上させます。
...
| `data.name` | String | - | "" | **🆕 ノードラベル名（上部に表示）** |
...
#### 下位互換性
- v2.1.0の`name`フィールドはオプションです
- 既存のv2.0.0およびv1.0.0のJSONファイルは引き続き動作します
- `name`フィールドがない場合は空文字列として扱われます

#### 上位互換性
- v2.1.0で作成したファイルは、`name`フィールドを無視すればv2.0.0でも読み込めます
- ただし、ラベル名は表示されません
